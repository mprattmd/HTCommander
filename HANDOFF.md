...
4. **Phase 3 — Avalonia UI**:
   ...
   - ✅ **Live telemetry** — the shell polls `GET_HT_STATUS` (BASIC/20, every 1.5s while connected) and parses replies with Core `RadioHtStatus`, binding power/TX-RX/squelch/channel/RSSI/region/GPS into a Radio-tab status panel. Both ingredients are hardware-proven (transport delivered `00 02 80 14 00 80 01 00 00`; `RadioHtStatus` parses it); full end-to-end GUI confirmation was blocked by a wedged radio (see gotcha) — re-verify after a radio BT reset.
